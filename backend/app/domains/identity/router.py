...
from .schemas import Token, UserCreate, User as UserSchema, PasswordResetRequest, PasswordResetConfirm
...
async def register_user(
    *,
    ...
    user = result.scalars().first()
    if user:
        print(f"DEBUG REGISTER: User {user_in.email} already exists. Raising 400.")
        raise HTTPException(
            status_code=400,
            detail="The user with this email already exists in the system",
        )
    
    print(f"DEBUG REGISTER: Creating user {user_in.email}")
    ...
    if user.role in [UserRole.EXPERT, UserRole.MENTOR]:
        ...
        expert_profile = ExpertProfile(
            user_id=user.id,
            # Mentors are approved by default in this flow, Experts need KYC
