    ...
    useEffect(() => {
        const fetchBookings = async () => {
            try {
                ...
                // Filter bookings that can be disputed: CONFIRMED, IN_PROGRESS, COMPLETED
                const eligible = data.filter(b => 
                    [BookingStatus.CONFIRMED, BookingStatus.IN_PROGRESS, BookingStatus.COMPLETED].includes(b.status)
                );
            ...
            <div className="max-w-6xl mx-auto space-y-12 pb-24">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
                    <div className="space-y-4">
                        <div className="flex items-center gap-4 text-gold/60">
                            ...
                            <span className="text-[10px] font-bold uppercase tracking-[0.4em]">Trung tâm giải quyết</span>
                        </div>
                ...
                <div className="min-h-[500px]">
                    <AnimatePresence mode="wait">
                        {activeTab === 'dispute' && <DisputeTab key="dispute" bookings={bookings} loading={loading} />}
...
function DisputeTab({ bookings, loading }: { bookings: Booking[], loading: boolean }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.success("Khiếu nại của bạn đã được gửi. Admin sẽ sớm liên hệ giải quyết.");
            setSelectedBooking(null);
            setDisputeForm({ reason: '', description: '', contact_info: '' });
        } catch (error) {
            console.error("Dispute failed", error);
            toast.error("Gửi khiếu nại thất bại. Vui lòng thử lại sau.");
        } finally {
                            ...
                            >
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-6">
                                        <div className="w-12 h-12 border border-ivory-10 flex items-center justify-center text-gold/60 group-hover:bg-gold/10 group-hover:text-gold transition-all duration-700">
                                            <Calendar size={20} strokeWidth={1} />
                                        </div>
                                        <div>
                                            <p className="text-[13px] font-bold text-ivory uppercase tracking-widest">{booking.expert?.user?.full_name || 'Chuyên gia'}</p>
                                    ...
                                    <div className="flex items-center gap-4">
                                        <span className={`text-[9px] font-bold px-3 py-1 border uppercase tracking-wider ${
                                            booking.status === BookingStatus.COMPLETED ? 'border-teal-mid/30 text-teal-mid' : 'border-gold/30 text-gold'
                                        }`}>
                                            {booking.status}
                                        </span>
                    ...
                    {selectedBooking ? (
                        ...
                        >
                            <div className="flex items-center gap-4 mb-10 text-gold">
                                ...
                                <h3 className="font-serif text-2xl italic">Chi tiết khiếu nại</h3>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-8">
                                <div className="space-y-3">
                                    ...
                                    >
                                        ...
                                        <option value="EXPERT_NO_SHOW">Chuyên gia không tham gia</option>
                                        <option value="QUALITY_ISSUE">Chất lượng chuyên môn không tốt</option>
                                        <option value="TECHNICAL_DIFFICULTY">Sự cố kỹ thuật từ hệ thống</option>
                                        <option value="INAPPROPRIATE_BEHAVIOR">Hành vi không chuẩn mực</option>
...
function FAQTab() {
    const faqs = [
        ...
        {
            q: "Tại sao tôi không thể vào phòng họp trực tuyến?",
            a: "Nút 'Hành lang họp' chỉ hiển thị trước giờ bắt đầu 10 phút. Nếu bạn vẫn không thấy, vui lòng kiểm tra lại kết nối mạng hoặc liên hệ hỗ trợ kỹ thuật.",
            steps: [
                "Đảm bảo bạn đã đăng nhập đúng tài khoản.",
                "Kiểm tra lịch hẹn tại trang 'Lịch trình' xem đã được chuyên gia xác nhận (CONFIRMED) chưa.",
                "F5 lại trang Dashboard để cập nhật trạng thái mới nhất."
            ]
        }
    ];
                    ...
                    <div className="flex gap-8">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="pt-6 border-t border-ivory-10">
                                <p className="text-[10px] font-bold text-gold uppercase tracking-[0.3em] mb-6">Các bước thực hiện đơn giản:</p>
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    {faq.steps.map((step, sIdx) => (
                                        <div key={sIdx} className="space-y-3">
                                            <span className="text-[9px] font-bold text-ivory-20 uppercase tracking-widest">Bước 0{sIdx + 1}</span>
    ...
    const guides = [
        {
            ...
            steps: [
                ...
                "Quét mã QR SePay để thanh toán tự động. Tiền sẽ vào tài khoản sau 30-60 giây."
            ]
        },
        {
            ...
            steps: [
                ...
                "Nhập ghi chú yêu cầu và xác nhận đặt chỗ. Đợi chuyên gia đồng ý (Confirm)."
            ]
        },
        {
            title: "Đánh giá & Hoàn tất buổi học",
            icon: <Star size={24} />,
            steps: [
                "Sau khi kết thúc buổi họp, vào trang 'Lịch trình'.",
                "Nhấn nút 'Hoàn tất buổi' và để lại đánh giá (Star Rating).",
                "Lưu ý: Ngay khi bạn nhấn 'Hoàn tất', Credits sẽ được chuyển cho chuyên gia. Chỉ nhấn khi bạn thực sự hài lòng."
            ],
            warning: "Cảnh báo quan trọng: Hành động 'Hoàn tất buổi' là không thể đảo ngược. Nếu có vấn đề không hài lòng, vui lòng sử dụng tính năng 'Khiếu nại' thay vì nhấn Hoàn tất."
        }
    ];
        ...
        >
            {guides.map((guide, index) => (
                <div key={index} className="bg-obsidian border border-ivory-10 p-10 flex flex-col justify-between group hover:border-gold-line transition-all duration-700 shadow-xl">
                    ...
                    {guide.warning && (
                        <div className="mt-10 p-4 bg-burgundy/10 border border-burgundy/30 flex gap-4 items-start">
                            <AlertCircle size={16} className="text-burgundy shrink-0 mt-0.5" />
                            <p className="text-[10px] text-burgundy font-medium uppercase tracking-wider leading-relaxed">
                                {guide.warning}
                            </p>
                        </div>
                    )}
            ...
            <div className="md:col-span-2 bg-gold/5 border border-gold/20 p-12 text-center space-y-6">
                <h4 className="font-serif text-2xl text-gold italic">Vẫn cần sự trợ giúp khác?</h4>
                <p className="text-ivory-45 text-sm font-light max-w-xl mx-auto tracking-wide">
                    Nếu các hướng dẫn trên chưa giải quyết được vấn đề của bạn, hãy kết nối trực tiếp với đội ngũ quản trị qua Chat hoặc Hotline.
                </p>
